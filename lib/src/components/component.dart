...
class Component {
  ...
  Future<void>? addToParent(Component parent) {
    print("add:${this} time:${FlameGame.myDt}");
    assert(
      _parent == null,
  ...
  void remove(Component component) {
    print("remove:${component} time:${FlameGame.myDt}");
  ...
  void _mount({Component? parent, bool existingChild = false}) {
    ...
    try {
      ...
      assert(_state == LifecycleState.loaded ||
          _state == LifecycleState.removed);
    } catch (e) {
      print("_mount:${this} state:${_state}");
      // GameLog.logD('_mount', "parent:${parent} this:${this}tag:${this.tag}");
     return;
    }
    assert(_parent != null && _parent!.isMounted);
...
class _LifecycleManager {
  ...
  void _processChildrenQueue() {
    while (_children.isNotEmpty) {
      ...
      if (child.isLoaded) {
        ...
        if (child._state != LifecycleState.uninitialized) {
          break;
        }else {
          child._load();
        }
  ...
  void _processRemovalQueue() {
    _removals.forEach((element) {
      print("removals element:${element}");
    });
    while (_removals.isNotEmpty) {
      final component = _removals.removeFirst();
      if (component.isMounted) {
        component._remove();
        print("${component} is removed");
      }else {
        print("${component} is not removed");
      }
